...
This error occurs when you specify an invalid API Key, or use a key from an explorer on a [**different chain**](../faq#can-the-same-api-keys-be-used-across-different-explorers).
...
To resolve, adhere to the [**rate limits**](rate-limits) of your available plan by waiting for a certain amount of time before each request. If you are using a script or application, **apply throttling** to limit the frequency of calls.
...
To resolve, consider selecting a smaller date/block range, though you may [**ping us**](getting-help) if you think the issue may be performance related.
